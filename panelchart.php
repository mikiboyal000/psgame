...
?>
 ...
 .panelbox{display: flex;
    justify-content: space-between;
    align-items: center;}
    .mynumber {padding: 0px;
    margin: 0px};
...
<?php
// `first_number`, `first_open_number`, `second_close_number`, `second_number`
$sql_r = "SELECT first_number as fn, `first_open_number`as oa, `second_close_number` as ca , second_number as sn,  date_format(`open_date`,'%Y-%m-%d') as d FROM `result` WHERE `result_game_id`='$id'";
$r = $con->query($sql_r);
$data =[];
...
$("#r"+i.d).html(`<div class="panelbox chart-${i.oa}${i.ca}">
<p class="pana mynumber">${i.fn}</p>
<p class="mid mynumber chart-${i.oa}${i.ca}">${i.oa}${i.ca}</p>
<p class="pana mynumber">${i.sn}</p>
</div>`);
