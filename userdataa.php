...
if(isset($_POST['appuser']) && isset($_POST['apppass']) && isset($_POST['fmctoken'])) {
    ...
    $sql = "SELECT `user_id`,  `status`, `play`,usrname,phone,  `notice1`, `pnotice`, `password`, `user_type`, `phonepe`, `paytm`, `gpay`, `bank_name`, `account_number`, `ifsc`,wallet.money as money,live FROM `user` INNER JOIN wallet on wallet.user_user_id = user_id WHERE (usrname ='$username' or  phone ='$username')  and password='$password' and user_type='user' ";
    $result = $con->query($sql);
...
//  Check user state Active Deactive
...
//  Check user state Active Deactive End
...
if(isset($_POST['appuserid']) && isset($_POST['myblanace'])) {
    ...
    $sql = "SELECT `wallet_id`, `money`, `user_user_id`, user.status,user.play,user.live FROM `wallet` inner join user on user.user_id = user_user_id  WHERE `user_user_id` ='$id' ";
    $result = $con->query($sql);
    ...
    if(mysqli_num_rows($result) == 1){
        ...
        $user["play"] =  $row['play'] =="active";
        $user["live"] =  $row['live'] =="1";
...
$data =json_decode($_POST['data'],true);;
    $values = "";
      ...
      $sql_check_time = "SELECT `id` FROM `game` WHERE `id`=$game_id and `game_on_off`='yes' and ".$open_close."_time >'$time' ";
      $result = $con->query($sql_check_time);
      if(mysqli_num_rows($result) == 1 ){
     mysqli_autocommit($con, false);
...
$total_bid_amount = 0;
  ...
  $my_balance_ref = (int)$myrow_ref['money'];
       ...
       $refprecent = 50;
       $total_ref= $my_balance_ref +$total_bid_amount/$refprecent;
          $sql_t_ref = "INSERT INTO `transaction`
            (`user_id`, `c_amount`, `credit`, `debit`, `final_amount`, `date`, `created_at`,comment,uids) 
            VALUES 
            ('$ref_id','$my_balance_ref',$total_bid_amount/$refprecent ,0,'$total_ref','$mydate','$date_time','Bonus point for bid placed','bonus_$uids')";
     if($con->query($sql_t_ref) === TRUE){
        //  money trasfer to refrance if 
           $sql2 ="UPDATE `wallet` SET `money`=money+$total_bid_amount/$refprecent ,`total_contribution`=total_contribution+$total_bid_amount  ,update_at='$date_time' WHERE `user_user_id`='$ref_id'";
  if($con->query($sql2) === TRUE){}
